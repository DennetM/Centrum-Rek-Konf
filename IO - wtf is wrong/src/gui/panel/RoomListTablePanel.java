...
public class RoomListTablePanel extends JPanel {
	...
	public void reloadData(List<RoomEquipment> rooms, boolean filtered) {
		...
		for (RoomEquipment rme : rooms) {
			Room rm = rme.getRoom();
			if (admin) {
				addRow(rm.getRoomId(), rm.getRoomNumber(), rm.getType(), rm.getSurface(), rm.getPeopleLimit(), rm.getAvailability());
			} else {
				addRow(rm.getRoomNumber(), rm.getType(), rm.getSurface(), rm.getPeopleLimit());
			}
		}
	...
	public RoomEquipment getRoomForNo(int no) {
		for (RoomEquipment re : listRooms) {
			if (re.getRoom().getRoomNumber() == no) {
				return re;
			}
		}
		return null;
